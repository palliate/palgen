...
class Generator:
    def __init__(
        self, config, out_path: str | Path = None, modules: list | None = None
    ):
        try:
            self.project = Project(config)
        except Exception as exception:
            logger.exception(
                "Could not parse project config. %s: %s.",
        ...
        self.out_path = (
            Path(out_path).resolve().absolute(
        ...
        if modules:
            for name, module in self.project:
                module.settings["enabled"] = name in modules
    ...
    def collect(self):
        for folder in self.project.folders:
            ...
            for config in [*Path.glob(self.project.root, folder + "/**/config.toml")]:
                logger.debug(
                    "Found config at %s", config.relative_to(self.project.root)
                )
